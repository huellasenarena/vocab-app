...
- **Reprise** (`saveRound`/`savedRound`/`resumeRound`, clé `vocab_round_state_<Langue>`, **exclue de la sync**) : on persiste surtout le **texte à trous** (`_clozeState`, avec ses trous déjà remplis) — le reste est léger. Écrit à chaque réponse, à chaque avance, sur `pagehide`, et à toute modification de la manche (mot jeté, mot renommé).
