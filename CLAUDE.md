...
- **La barre mesure la JOURNÉE, pas la manche** (`dayProgress`, « 6 sur 14 aujourd'hui ») : quatre mots le matin et quatre le soir sont deux manches, mais une seule journée. Total = mots faits (`doneToday`, **lu en base**) + mots restants de la manche + rappels dus hors manche + quota de neufs **non déjà tiré** dans la manche. ⚠️ `doneToday` vivait en localStorage, exclu de la sync : c'est ce doublon qui faisait dire « 12 / 16 » à l'iPhone et « 0 / 4 » à l'iPad le même matin. La donnée existait déjà dans `history` — on compte là, on ne synchronise pas une copie. Bénéfice : un mot travaillé en mode libre ou espacé compte lui aussi. ⚠️ Sans cette dernière soustraction, les mots neufs de la manche comptaient deux fois (15 au lieu de 8 sur une manche de huit). Le calendrier **« mes révisions à venir »** est offert en pratique comme en espacé.
