...
- **Deux curseurs, le troisième se déduit** : *mots par manche* et *mots neufs* ; les **rappels** valent la différence. ⚠️ Les régler tous les trois recrée deux quotas concurrents où le plus petit gagne (bug du 2026-09-09 : une manche à un seul mot). Les deux curseurs vivent sur l'**écran de départ** ; celui des neufs est le même `maxNewPerDay` qu'en ⚙️ (`setMaxNewPerDay` appelle `syncRoundDials`). Quota du jour épuisé → le curseur se grise (`.dial-spent`) mais reste manœuvrable, le monter rouvre des places aujourd'hui.
...
- **Les deux curseurs sont des PLAFONDS** (2026-09-20) : la manche sert les rappels dans les places que la réserve ne leur prend pas, puis les mots neufs **dans leur réserve et pas au-delà**. Une manche peut donc faire **moins** que sa taille ; elle ne sert jamais plus de mots neufs que son chiffre. ⚠️ Les neufs prenaient auparavant **tout ce que les rappels laissaient libre** (« la manche fait toujours son compte ») : réglé sur *8 mots dont 1 neuf* avec seulement 3 rappels dus, on recevait 3 rappels et **5 mots neufs**, et le quota du jour sautait avec. Une manche courte est la bonne réponse — la carte de fin propose d'ouvrir des mots neufs en plus, et c'est un **clic conscient**, comme partout ailleurs dans ce mode. ⚠️ Mais on ne réserve que ce que le **stock** peut tenir (`reserve = min(quota, mots neufs disponibles)`) : une place gardée pour un mot neuf qui n'existe pas est une place perdue — vingt rappels en retard et une manche de quatre mots. Le bouton **« que des rappels »** met la réserve à zéro pour une manche, sans toucher au réglage.
...
- **Le quota de neufs se lit en base** (`todayNewCount`, table `sessions`). ⚠️ Le compteur local des neufs **affichés** (`vocab_today_new_served_*`) a été retiré : c'était le dernier chiffre de la journée à ne pas voyager. Un mot vu mais pas répondu n'a pas été fait — il peut revenir.
...
- ⚠️ **La fuite par l'accord ne vient pas des intrus, elle vient de la RÉSERVE ENTIÈRE** (2026-09-18). La règle ne portait que sur l'intrus face à la cible qu'il concurrence ; l'apprenant, lui, voit **une seule réserve** contenant toutes les cibles et tous les intrus. « besar una ___ que había saltado al jardín » avec un seul mot féminin dans toute la réserve se résout **sans connaître un seul sens**. Deux règles :
  ...
  - **l'article entre dans le trou** quand la cible est un nom : « quiso besar {3} » de forme `una rana`, jamais « besar una {3} ». Un article visible devant un trou livre le genre.
