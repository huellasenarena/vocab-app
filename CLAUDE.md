...
- **La séance est une MANCHE de 12 exercices** (`ROUND_EXERCISES`), et un exercice n'est pas un mot : un QCM en sert un, un texte à trous **jusqu'à huit d'un coup**. C'est ce regroupement qui rend **8 mots neufs par jour** tenable — comptés en mots, ils demanderaient ~25 items par jour.
- **Le nombre de mots neufs vient du réglage ⚙️** (`maxNewPerDay`, défaut `DEFAULT_NEW_PER_DAY` = 8, pas de 1 dans `MAX_NEW_VALUES` jusqu'à 10). ⚠️ Une **constante du mode en plus du réglage** donnait deux quotas concurrents et le plus petit gagnait : le 2026-09-09, une manche à **un seul mot** dans les quatre langues, parce que le réglage valait 1 du temps du mode espacé. Le quota retient le **plus grand** des deux compteurs « neufs du jour » — ceux affichés en séance (local) et ceux sortis du pool (D1, alimenté aussi par « étudier ») — sinon huit mots appris ailleurs le matin en rouvriraient huit le soir.
...
- **La barre mesure la JOURNÉE, pas la manche** (`dayProgress`, « 6 sur 14 aujourd'hui ») : quatre mots le matin et quatre le soir sont deux manches, mais une seule journée. Total = mots faits (`vocab_today_done_<Langue>_<date>`, local, hors sync) + mots restants de la manche + rappels dus hors manche + quota de neufs **non déjà tiré** dans la manche. ⚠️ Sans cette dernière soustraction, les mots neufs de la manche comptaient deux fois (15 au lieu de 8 sur une manche de huit). Le calendrier **« mes révisions à venir »** est offert en pratique comme en espacé.
...
### Reste à faire
Tirage par groupe au démarrage · lien « pourquoi ? » sur un trou raté · budget de la manche réglable dans ⚙️ (le nombre de mots neufs l'est déjà) · **régulation automatique du quota** (idée, non tranchée le 2026-09-09) : ne plus ouvrir de mots neufs tant que plus de ~15 rappels sont en retard — à 8 neufs/jour il faut ~2 manches, et une seule manche par jour endette indéfiniment ; ça rendrait le réglage impossible à mal choisir · **carte de langue de l'accueil** : elle additionne encore les mots dus + le quota de neufs, c'est-à-dire le pool du mode espacé, et annonce « 340 » quand la manche en sert 19. **Forme arrêtée le 2026-09-09 sur page de test** (https://claude.ai/code/artifact/2cbeb644-6fb3-454a-b2d5-fa0a7445fbd0) : **fraction** (« 8 / 20 », soit `dayProgress()` par langue) + **anneau autour du drapeau** + **doré et vert** (le rouge disparaît : à 8 mots neufs par jour, une carte rouge tous les matins ne veut plus rien dire). Reste à écrire dans `updateLangBadges` · **fondre le mode espacé dans le mode libre** (pool « à réviser aujourd'hui » à côté des groupes virtuels existants ; le calendrier des révisions se rattache au mode pratique) · retrait des `console.log` `[Séance]` · plus tard : écriture en clôture de groupe, mémoire des QCM en D1.
