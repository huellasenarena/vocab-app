...
**Onglets** : `English`, `Spanish`, `French`, `Greek`, `Progress`, `History`, `Session`, `Tokens`, `Blacklist`. Mots dans colonne B (timestamp en A) pour chaque onglet langue.
...
**Sheet formes grammaticales** : onglet `Spanish`, row 1 = catégories (presente, pasado, futuro, subjuntivo, condicional, imperativo, misc.), rows 2+ = formes. Aplati côté JS en `"${cat} ${forme}"` (ex: `"futuro simple"`, `"subjuntivo pluscuamperfecto"`).
