...
- **Deux curseurs, le troisième se déduit** : *mots par manche* et *mots neufs* ; les **rappels** valent la différence. ⚠️ La ligne déduite se calcule sur **ce qui reste du quota du jour** (`maxNewPerDay − todayNewCount`), **pas sur le réglage** : les neufs sont un quota du **jour**, donc s'ils ont déjà été servis ce matin il n'en reste aucun à réserver et les rappels prennent toutes les places. La ligne annonçait « → 2 rappels » et la manche en servait 4 (bug du 2026-09-18) ; elle dit maintenant « → 4 rappels · 2 neufs déjà faits ». ⚠️ Les régler tous les trois recrée deux quotas concurrents où le plus petit gagne (bug du 2026-09-09 : une manche à un seul mot). Les deux curseurs vivent sur l'**écran de départ** ; celui des neufs est le même `maxNewPerDay` qu'en ⚙️ (`setMaxNewPerDay` appelle `syncRoundDials`).
- **Le curseur des neufs RÉSERVE des places, il ne plafonne pas la journée** : `buildSessionItems(reviewsOnly)` sert les rappels d'abord dans la limite `taille − réservé`, puis les mots neufs pour **tout ce qui reste**. Trois rappels en attente le soir → 3 rappels + 9 neufs, la manche fait toujours son compte. Le bouton **« que des rappels »** met la réserve à zéro pour une manche, sans toucher au réglage.
