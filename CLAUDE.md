...
### Reste à faire
Tirage par groupe au démarrage · lien « pourquoi ? » sur un trou raté · budget de la manche réglable dans ⚙️ (le nombre de mots neufs l'est déjà) · **régulation automatique du quota** (idée, non tranchée le 2026-09-09) : ne plus ouvrir de mots neufs tant que plus de ~15 rappels sont en retard — à 8 neufs/jour il faut ~2 manches, et une seule manche par jour endette indéfiniment ; ça rendrait le réglage impossible à mal choisir · **carte de langue de l'accueil** : elle additionne encore les mots dus + le quota de neufs, c'est-à-dire le pool du mode espacé, et annonce « 340 » quand la manche en sert 19 (page de test à faire avant de toucher au visuel) · **fondre le mode espacé dans le mode libre** (pool « à réviser aujourd'hui » à côté des groupes virtuels existants ; le calendrier des révisions se rattache au mode pratique) · retrait des `console.log` `[Séance]` · plus tard : écriture en clôture de groupe, mémoire des QCM en D1.
