...
### Reste à faire
Tirage par groupe au démarrage · lien « pourquoi ? » sur un trou raté · budget de la manche réglable dans ⚙️ (le nombre de mots neufs l'est déjà) · **carte de langue de l'accueil** : elle additionne encore les mots dus + le quota de neufs, c'est-à-dire le pool du mode espacé, et annonce « 340 » quand la manche en sert 19 (page de test à faire avant de toucher au visuel) · **fondre le mode espacé dans le mode libre** (pool « à réviser aujourd'hui » à côté des groupes virtuels existants ; le calendrier des révisions se rattache au mode pratique) · retrait des `console.log` `[Séance]` · plus tard : écriture en clôture de groupe, mémoire des QCM en D1.
